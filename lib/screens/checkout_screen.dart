...
// Hàm xử lý thay đổi địa chỉ
  ...
  Future<void> placeOrder(List<OrderItem> orderItems) async {
    String note = noteController.text;
    try {
      await apiOrdersService.addOrderOnline(
        defaultAddress!.tenNguoiNhan,
        defaultAddress!.soDienThoaiNguoiNhan,
        defaultAddress!.diaChiNguoiNhan,
        defaultAddress!.xaPhuongNguoiNhan,
        defaultAddress!.quanHuyenNguoiNhan,
        defaultAddress!.tinhThanhNguoiNhan,
        paymentMethod,
        note,
        selectedDiscount,
        orderItems,
      ...
      print('ten nguoi nhan ${defaultAddress!.tinhThanhNguoiNhan}');
  ...
  Widget build(BuildContext context) {
    ...
    // Chuyển đổi selectedItems thành List<OrderItem>
    final List<OrderItem> orderItems = selectedItems.map((cartItem) {
    ...
    // Tính tổng khối lượng của các sản phẩm đã chọn
    subtotalWeight = selectedItems.fold(
      0.0,
          (sum, item) => sum + parseKhoiLuong(item.khoiLuong) * item.soLuong,
    );
                ...
                Column(
                  children: selectedItems.map((item) {
                    return _buildProductItem(
                      ...
                      price: '${formatCurrency(item.gia)} đ',
